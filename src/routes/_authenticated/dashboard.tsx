  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        {isLandlord ? (
          <LandlordDashboard userId={session!.user.id} />
        ) : (
          <TenantDashboard userId={session!.user.id} />
        )}
      </div>
    </div>
  );
  ...
  const { data: bookings } = useQuery<Booking[]>({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("bookings")
        .select(
          "id, start_date, end_date, status, message, tenant_id, listings(id, title, city, images, landlord_id), profiles(id, full_name)",
        )
        .eq("tenant_id", userId)
          ...
          {bookings.map((b) => (
            ...
            >
              <img
                src={
                  b.listings?.images?.[0] ||
                  "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=200&q=60"
                }
                alt=""
  ...
  const { data: bookings } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("bookings")
        .select(
          "id, start_date, end_date, status, message, tenant_id, listings!inner(id, title, landlord_id)",
        )
              ...
              <div key={l.id} className="flex gap-4 rounded-2xl border bg-card p-4 shadow-card">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => deleteListing(l.id)}>
                      Delete
                    </Button>
                  </div>
                </div>
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-card">
          {!bookings || bookings.length === 0 ? (
            <EmptyState
              ...
              body="Booking requests from tenants will appear here."
            />
          ) : (
            <div className="space-y-3">
              {bookings.map((b: any) => (
                <div
                  key={b.id}
                  className="flex flex-wrap items-start gap-4 rounded-xl border bg-background p-4"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {b.message && (
                      <p className="mt-2 rounded-lg bg-muted px-3 py-2 text-sm">{b.message}</p>
                    )}
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge status={b.status} />
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge className={`${variant[status] ?? "bg-muted"} border-0 capitalize`}>{status}</Badge>;
}
